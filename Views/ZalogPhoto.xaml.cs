...
public partial class ZalogPhoto : ContentPage
{
    private Zalogi _selectedZalog;
    private Zavkr _selectedZavkr;
    private string _latitude;
    private string _longitude;
    private string _innZavkr;
    public ZalogPhoto(Zalogi selectedZalog)
    {
        InitializeComponent();
        _selectedZalog = selectedZalog;
        _ = LoadSessionDataAsync();
        totalNum.Text = "Загрузка количества фотографий";
        LoadDataFromService();
    }
    ...
    private async void OnTakePhotoButtonClicked(object sender, EventArgs e)
    {
        ...
        if (photoData != null)
        {
            ...
            if (result == "OK")
            {
                await DisplayAlert("Успех", "Фотография отправлена", "OK");
            }
        }
    ...
    private async void OnTakeGalleryPhotoClicked(object sender, EventArgs e)
    {
        ...
        if (photoData != null)
        {
            ...
            string ZvPozn =  _selectedZalog.DgPozn.ToString();
            var saved =   MauiApp1.Platforms.Android.Services.SavePictureService.SavePicture(photoData, namePic, ZvPozn);

            if (saved)
            {
                await DisplayAlert("Успех", "Фотография сохранена в галерею", "OK");
            }
            else
            {
    ...
    private async void OnUploadPhotoClicked(object sender, EventArgs e)
    {
        ...
        if (photo != null)
        {
            ...
            if (parts.Length >= 2)
            {
                ...
                if (result == "OK")
                {
                    await DisplayAlert("Успех", "Фотографии успешно отправлены", "OK");

                }
            }
            else
            {
                await DisplayAlert("Ошибка", "Фото бещ локации", "OK");
            }
            loadingIndicator.IsRunning = false;
            loadingIndicator.IsVisible = false;
        }
    ...
    private async void LoadDataFromService()
    {
        ...
        string ZvPozn = _selectedZalog.DgPozn.ToString();
        string id = _selectedZalog.ZalogId.ToString().Trim();
        string path = $"{inn}\\ZAVKR\\{ZvPozn}\\������\\{id}";
