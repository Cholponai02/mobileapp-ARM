...
public partial class ActivityPhoto_3 : ContentPage
{
    private Zavkr _selectedZavkr;
    private string _latitude;
    private string _longitude;

    public ActivityPhoto_3(Zavkr selectedZavkr)
    {
        InitializeComponent();
        _selectedZavkr = selectedZavkr;
        _ = LoadSessionDataAsync();
        totalNum.Text = "загрузка фотографий";
        LoadDataFromService();
    }
    ...
    private async void OnTakePhotoButtonClicked(object sender, EventArgs e)
    {
        ...
        if (photoData != null)
        {
            ...
            if (result == "OK")
            {
                await DisplayAlert(" Успех", "Успешно отрпавлено", "OK");
            }
        }
    }
    ...
    private async void OnTakeGalleryPhotoClicked(object sender, EventArgs e)
    {
        ...
        if (photoData != null)
        {
            ...
            if (saved)
            {
                await DisplayAlert("Успеч", "Успешно сох", "OK");
            }
            else
            {
                await DisplayAlert("Ошибка", "Фото без локации", "OK");
            }
#endif
        }
    }
    ...
    private async void OnUploadPhotoClicked(object sender, EventArgs e)
    {
        ...
        if (photo != null)
        {
            ...
            {
                await DisplayAlert("Ошибка", "Фотографии без локации", "OK");
            }
            loadingIndicator.IsRunning = false;
            loadingIndicator.IsVisible = false;
        }
    ...
    private async void LoadDataFromService()
    {
        string inn = _selectedZavkr.Inn;
        string ZvPozn = _selectedZavkr.PositionalNumber.ToString();
        string path = $"{inn}\\ZAVKR\\{ZvPozn}\\ActivityPhoto3";
        ...
        if (response.IsSuccessStatusCode)
        {
            ...
            {
                totalNum.Text = "не известно колич.фото";
            }
        }
    }
